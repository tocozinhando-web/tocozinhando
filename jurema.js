...
class Jurema {
  ...
  /* ============================================================
     PRIVADO: detecta o caminho base correto para qualquer página
  ============================================================ */
  _detectBasePath(override) {
    if (override) return override;

    // Tenta inferir pela URL atual
    // Se estiver em /receita.html, /receitas.html etc → raiz é ./
    // Se estiver em /sobre.html → raiz é ./
    // O path sempre relativo à raiz do site
    const path = window.location.pathname;

    // Se a URL tem subpastas (ex: /pages/receita.html), ajusta
    const depth = (path.match(/\//g) || []).length - 1;
    const prefix = depth > 0 ? '../'.repeat(depth) : '';

    return prefix + 'assets/imagens/jurema/';
  }
